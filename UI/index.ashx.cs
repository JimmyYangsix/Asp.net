    ...
    public class index : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            foreach(UserInfo  user in list){
                sb.AppendFormat("<tr class=" + className[i % 5] + "><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td><a href='DeleteUser.ashx?id={0}'>删除</a></td>", user.Id,user.UserName,user.sid,user.UserPass,user.Direction,user.Class);
                i++;
            }
